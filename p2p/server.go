...
func (srv *Server) Peers() []*Peer {
	...
	srv.doPeerOp(func(peers map[enode.ID]*Peer) {
		for _, p := range peers {
			ps = append(ps, p)
			//fmt.Println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! p2p/server.go -> Peers(피어는 모든 피어를 반환합니다.) :", ps)
		}
	})
	return ps
}
...
func (srv *Server) AddPeer(node *enode.Node) {
	...
	//fmt.Println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! p2p/server.go -> AddPeer :", node)
}
	...
	for {
		...
		case c := <-srv.checkpointAddPeer:
			...
			if err == nil {
				...
				//fmt.Println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! p2p/server.go run() -> peercount", reflect.TypeOf(string(strconv.Itoa(int(len(peers))))), "id", reflect.TypeOf(p.ID()), "conn", reflect.TypeOf(string(c.flags)), "addr", reflect.TypeOf(p.RemoteAddr()), "name", reflect.TypeOf(truncateName(c.name)))
