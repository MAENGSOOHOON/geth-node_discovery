...
func (t *UDPv4) findnode(toid enode.ID, toaddr *net.UDPAddr, target encPubkey) ([]*node, error) {
	...
	rm := t.pending(toid, toaddr.IP, p_neighborsV4, func(r interface{}) (matched bool, requestDone bool) {
		...
		for _, rn := range reply.Nodes {
			t.log.Trace("Invalid candidate neighbor node list received ", "ip.UDP", rn.UDP, "ip.TCP", rn.TCP, "ip", rn.IP, "ID", toaddr, "addrIP", toaddr.IP)

			nreceived++
			n, err := t.nodeFromRPC(toaddr, rn)
/*
			fmt.Println(reflect.TypeOf(string(toaddr.IP)))
			fmt.Println(reflect.TypeOf(string(rn.IP)))
		
			source := string(toaddr.IP)
			source1 := string(rn.IP)

			fmt.Println(source)
			fmt.Println(source1)
			
			fmt.Println("################################################## toaddr :", toaddr,"+ rn.IP", rn.IP)
*/		
			//t.log.Trace("Invalid candidate neighbor node list received ", "ip.UDP", rn.UDP, "ip.TCP", rn.TCP, "ip", rn.IP, "ID", toaddr, "addrIP", toaddr.IP)
			if err != nil {
